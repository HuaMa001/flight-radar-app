        ...
        if matched:
            ...
            if details:
                ...
                return {
                    ...
                    "高度 (ft)": details["alt"],
                    "地速 (kts)": details["spd"],
                    "起飛台灣": "🇹🇼 台灣起飛" if is_origin_taiwan else "否",
                    "降落台灣": "🇹🇼 降落台灣" if is_dest_taiwan else "否",
                    "資料來源": "📡 直播廣播",
                    "lat": details["lat"],
                    "lon": details["lon"],
                    "_is_taiwan": is_taiwan_related,
                }
...
if not df_matched.empty:
    st.info(
        "💡 **點擊下方表格任意航班，表格會自動載入照片                         is_origin_taiwan = check_is_taiwan(origin)
                            is_dest_taiwan = check_is_taiwan(destination)
                            is_taiwan_related = is_origin_taiwan or is_dest_taiwan
