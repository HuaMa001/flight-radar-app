...
def check_is_taiwan(text_or_code: str) -> bool:
    ...
    if s in tw_airport_codes or (len(s) == 4 and s.startswith("RC")): return True
...
def is_taiwan_registration(reg: str) -> bool:
    if not reg or reg == "未知": return False
...
def fetch_direct_clickhandler(fr_api_inst, flight_obj_or_id) -> dict | None:
    try:
        ...
        origin = orig_obj.get("iata") or orig_obj.get("icao") or (airport.get("origin") or {}).get("name") or "未知"
        destination = dest_obj.get("iata") or dest_obj.get("icao") or (airport.get("destination") or {}).get("pluginData", {}).get("details", {}).get("name") or "未知"
        ...
        return {
            ...
            "f_num": f_num, "f_reg": f_reg, "ac_code": ac_code,
            "arr_ts": arr_ts, "eta_time": eta_full,
            "image_url": image_url,
        }
    except Exception:
        return None
...
def build_flight_index(all_flights: list) -> dict:
    ...
    for f in all_flights:
        ...
        for val, key in [(f_reg, "registration"), (f_num, "number"), (f_callsign, "callsign")]:
            if val:
                index[key][val] = f
                if norm := normalize_target(val): index["normalized"][norm] = f
    return index
...
def find_target_in_index(target: str, flight_index: dict):
    ...
    if t_norm and (f := flight_index["normalized"].get(t_norm)): return f, "NORMALIZED_MATCH"
    return None, None
...
def build_result(target_raw: str, flight, details: dict, source: str) -> dict | None:
    ...
    return {
        ...
        "eta_time": details["eta_time"], "arr_ts": arr_ts,
...
def scan_taiwan_airport_schedules(unmatched_targets: list) -> dict:
    ...
    for apt in airports:
        ...
        try:
            ...
            arrivals = res.json().get("result", {}).get("response", {}).get("airport", {}).get("pluginData", {}).get("schedule", {}).get("arrivals", {}).get("data", [])
            for arr in arrivals:
    ...
    for fetch_by in ["reg", "flight"]:
        ...
        try:
            ...
            if tw_flights: best_flight = min(tw_flights, key=lambda x: abs(x[1] - current_ts))[0]
                ...
                recent_flights = [(f, ts, dest) for f, ts, dest in valid_flights if abs(ts - current_ts) <= (24 * 3600)]
...
def send_discord_webhook(taiwan_flights: list):
    ...
    for category, flights in grouped_flights.items():
        ...
        if not target_webhook:
            print(f"⚠️ 未設定 [{category}] 的 Webhook URL，跳過該分類共 {len(flights)} 架班機。")
            continue
        ...
        for i in range(0, len(embeds), 10):
            ...
            try:
                ...
                if res.status_code in [200, 204]:
                    ...
                    print(f"❌ [{category}] Discord 發送失敗，狀態碼: {res.status_code}")
            except Exception as e:
        ...
        for future in as_completed(future_to_target):
            ...
            try:
                if res := future.result():
                    results[target] = res
                    print(f"  └─ 🟢 [補查成功] {target} -> {res['f_num']} ({res['route']}) | 🕒 {res['eta_time']}")
                else:
                    print(f"  └─ ⚪ [無結果] {target}")
            except Exception as e:
    ...
    for target in TARGETS:
        ...
        if flight and (details := fetch_direct_clickhandler(fr_api_inst, flight)):
            ...
            if res:
                ...
                print(f"  └─ 🟢 [Live掃描] {target} -> {res['f_num']} ({res['route']}) | 🕒 {res['eta_time']}")
                continue
